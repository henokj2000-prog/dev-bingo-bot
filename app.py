...
def init_db():
    ...
    if not admin:
        ...
        db.execute('INSERT INTO admins (username, password_hash, is_super, created_at) VALUES (?, ?, ?, ?)',
                   ("superadmin", hashed, 1, time.time()))
    db.commit()
    db.close()
...
# -------------------- Game routes (unchanged except any minor adjustments) --------------------
...
def admin_login():
    ...
    if admin and check_password_hash(admin['password_hash'], password):
        ...
        return jsonify({'success': True, 'is_super': session['admin_is_super']})
...
def admin_check():
    if 'admin_id' in session:
        return jsonify({'logged_in': True, 'is_super': session.get('admin_is_super', False)})
    return jsonify({'logged_in': False}), 401

# All admin API endpoints now use @admin_required decorator
@app.route('/admin')
def admin():
